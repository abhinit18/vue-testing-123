...
module.exports = ".annotator-hide{display:none;visibility:hidden}"
...
var Adder = Widget.extend({
    ...
    constructor: function (options) {
        ...
        this.reverseHighlightMap = util.reverseObject(this.options.highlightClassMap);
        ...
        this
            .element
            .on("click." + NS, '.colorBox', function (e) {
             //   debugger;
                ...
                if (className.indexOf('blue') != -1) {
                //    self.turnHighlight('Blue')
                    currentHighlight = self.options.highlightClassMap['Blue'];
                } else if (className.indexOf('orange') != -1) {
                    currentHighlight = self.options.highlightClassMap['Orange'];
                  //  self.turnHighlight('Orange')
                } else if (className.indexOf('green') != -1) {
                    currentHighlight = self.options.highlightClassMap['Green'];
                   // self.turnHighlight('Green')
                }

                if (self.annotation.length > 0 && Array.isArray(self.annotation)) {

                    var optionsColor = {
                        colorClass: currentHighlight
                    }
                    self.annotation[0].options = optionsColor;
                    self
                        .options
                        .update(self.annotation[0])

                    return;
                }

                // Create a new annotation
                if (self.annotation !== null && typeof self.onCreate === 'function') {

                    var options = {
                        colorClass: currentHighlight
                    }
                    self.annotation.options = options;
                    self.onCreate(self.annotation, e);

                    self.hide();
                    self.removeHighlight('Default')

                }
            })
            .on("click." + NS, '.addNoteBox', function (e) {		
				
				if(self.annotation[0] != undefined){
					self._onDeleteClick(e);

					self.annotation = self.annotation[0];

					var newAnn = {quote: self.annotation.quote, ranges: self.annotation.ranges}

					self.hide();
					e.stopImmediatePropagation();
					e.preventDefault();
					self.onCreate(newAnn, e);
            ...
            .on("click." + NS, '.noteBox', function (e) {
                self._onClick(e);
			})
            // .on("mousedown." + NS, '.noteBox', function (e) {
            //     //self._onMousedown(e);
            // })
            .on("click." + NS, '.noteRemove', function (e) {
                self._onDeleteClick(e);
            });

        this.document = this.element[0].ownerDocument;
        $(this.document.body).on("mouseup." + NS, function (e) {
            //console.log('On mouse up adder')
            return false;
            self._onMouseup(e);
        });
    },
    ...
    //
    // annotation - An annotation Object to load. position - An Object specifying
    // the position in which to show the editor            (optional).
    //
    ...
    //
    // Returns nothing.
    load: function (annotation, position, isNew) {

        if (isNew) {
            $('.highlightOpt').addClass('hideBox');
            ...
            $(this.element)
                .find('.blue').addClass('active')

            this
                .element
                .find('.noteRemove')
                .addClass('removeRemoveNote');
        } else {

            if ($('.highlightOpt').hasClass('hideBox')) {
                $('.highlightOpt').removeClass('hideBox')
            }

            if (annotation[0]['options'] && annotation[0]['options']['colorClass']) {
                $(this.element)
                    .find('.colorBox')
                    .removeClass('active');

                $('.' + this.reverseHighlightMap[annotation[0]['options']['colorClass']].toLowerCase()).addClass('active')
            }

            this
                .element
                .find('.noteRemove')
                .removeClass('removeRemoveNote');
        }
    ...
    // Examples
    //
    //   adder.show()   adder.hide()   adder.show({top: '100px', left: '80px'})
    //
    // Returns nothing.
    show: function (position) {
        if (typeof position !== 'undefined' && position !== null) {
            ...
            if(window.article){
                ...
                if (position.left > crux) {
                    var subtractPos = position.left - crux;
                    leftPos = position.left - subtractPos
                } else leftPos = position.left
            }
            else {
				if(window.platform == "ANDROID"){
					...
					var currentPage = parseInt(currentLocation.start.displayed.page);
					var crux = 0;
					var akash = window.parent.document.getElementById('area').clientWidth;
						if( currentPage == 1)
							crux = 20;
						else crux = (currentPage - 1 ) * (akash);
					leftPos =  crux
					this
                .element
                .css({marginLeft: 40})
				} else if(window.platform != "IOS"){
					crux = position.width < 768 ? 100 : 868;

					if (position.screenX > crux) {
							var subtractPos = position.screenX - crux;
							leftPos = position.left - subtractPos
					} else leftPos = position.left
					            ...
					            if (typeof position !== 'undefined' && position !== null) {
						                ...
						                var crux = 0;
						                    if( currentPage == 1)
						                        crux = 0;
						                    else crux = (currentPage - 1 ) * window.screen.width - 10;
						                var leftPos = 0 + crux
    ...
    _onMousedown: function (event) {
        ...
        // event.preventDefault(); Prevent the selection code from firing when the mouse
        // button is released
    ...
    _onClick: function (event) {
        ...
        if (event.currentTarget.className.indexOf('addNoteBox') > -1) {
            this.hide();
            event.stopImmediatePropagation();
            event.preventDefault();
        ...
        //   event.preventDefault(); Hide the adder this.hide();
        this.ignoreMouseup = false;

        $('.highlightOpt').removeClass('hideBox');

        //this.turnHighlight('Blue')
...
Adder.template = [
    ...
    '<div class="annotatorInner">     ' +
    '   <div class="noteBox" style="-webkit-user-select:none;">' +
    '      <span class="noteIcon"  onselectstart="return false"></span>       ' +
    '      <span class="highlightSpan" onselectstart="return false">' + window.translationsReader['highlight'] +'</span>         ' +
    '      <div class="highlightOpt hideBox">' +
    '       <span class="colorBox blue active" onselectstart="return true"></span> ' +
    '       <span class="colorBox orange" onselectstart="return true"></span>' +
    '       <span class="colorBox green" onselectstart="return true"></span>' +
    '       <span class="noteRemove removeRemoveNote"  onselectstart="return true">' + window.translationsReader['clear'] +'</span>' +
    '      </div>' +
    '   </div>' +
    '<div class="diviDer"></div>' +
    '   <div class="noteBox addNoteBox" style="-webkit-user-select:none;">  ' +
    '      <span class="noteIcon"></span> ' +
    '      <span class="noteSpan">' + window.translationsReader['comment'] +'</span>' +
    '   </div>  ' +
    '</div>' +
    '</div>'].join('\n');
...
Adder.options = {
    // Callback, called when the user clicks the adder when an annotation is loaded.
    onCreate: null,
    highlightClassMap: {
        "Blue": "annotator-hl-blue",
        "Green": "annotator-hl-green",
        "Orange": "annotator-hl-orange"
    }
};
...
var Editor = exports.Editor = Widget.extend({
    ...
    //
    // Returns nothing.
    show: function (position, isNew) {

        if(window.platform == 'web' || window.article ){
			if (typeof position !== 'undefined' && position !== null) {
				var topPos = 0;
				if(window.commonArticle){
					//debugger;
						if( position.top > $('.articalSecDiv').height() - 320 && (position.top - (position.top - ($('.articalSecDiv').height() - 320)) > 0))
							topPos = position.top - (position.top - ($('.articalSecDiv').height() - 320))
						else topPos = position.top
				} else {
					if(position.top > 49)
						topPos = 49
			...
			if (typeof position !== 'undefined' && position !== null) {
				...
				var currentLocation = window.rendition.currentLocation();
							var currentPage = parseInt(currentLocation.start.displayed.page);
							var extraWidth = window.parent.document.getElementById('area').clientWidth;
								if( currentPage == 1)
									crux = 0;
								else crux = (currentPage - 1 ) * extraWidth
					...
					$(this.element).find('.annotator-widget').css('min-width', '200px')
        ...
        if(isNew){
            this.element
            .find('.annotator-save')
            .addClass('disabledSave');

            this.element
            .find('.annotator-cancel')
            .addClass('removeRemove');
            
        } else {
            this.element
            .find('.annotator-save')
            .removeClass('disabledSave');

            this.element
            .find('.annotator-cancel')
            .removeClass('removeRemove');
        }
...
Editor.template = [
    ...
    '   <div class="head"> <span class="aclose">&times;</span> <span class="noteIcon"></span> ' + window.translationsReader['addNote'] +'</div>',
    '    <ul class="annotator-listing"></ul>',
    '    <div class="annotator-controls">',
    '     <a href="#cancel" class="annotator-cancel">' + window.translationsReader['remove'] +'</a>',
    '      <a href="#save"',
    '         class="annotator-save annotator-focus">' + window.translationsReader['save'] +'</a>',
    '    </div>',
    '  </form>',
    '</div>'
].join('\n');
...
exports.standalone = function standalone(options) {
    ...
    return {
        ...
        beforeAnnotationCreated: function (annotation, eventAnnotation, optionAnnotation) {
            return widget.load(annotation, eventAnnotation, optionAnnotation);
        },
...
function highlightRange(normedRange, cssClass, addClass) {
    if (typeof cssClass === 'undefined' || cssClass === null) {
        cssClass = 'annotator-hl';
    }
    if (typeof addClass === 'undefined' || addClass === null || !addClass) {
        addClass = ' annotator-hl-ann';
    } else addClass = addClass.colorClass
    ...
    for (var i = 0, len = nodes.length; i < len; i++) {
        ...
        if (!white.test(node.nodeValue)) {
            ...
            hl.className = cssClass + ' ' + addClass;
...
Highlighter.prototype.draw = function (annotation) {
    ...
    for (var j = 0, jlen = normedRanges.length; j < jlen; j++) {
        ...
        $.merge(
            annotation._local.highlights,
            highlightRange(normed, this.options.highlightClass, annotation.options)
        );
    }
...
function main(options) {
    ...
    function start(app) {
        ...
        s.adder = new adder.Adder({
            onCreate: function (ann, annEvent, annOption) {
                app.annotations.create(ann, annEvent, annOption);
            },
            onDelete: function (ann) {
                app.annotations['delete'](ann);
                s.highlighter.undraw(ann);
        ...
        s.textselector = new textselector.TextSelector(options.element, {
            onSelection: function (ranges, event) {
                if (ranges.length > 0) {
                    ...
                    s.interactionPoint = util.mousePosition(event, true);
                    s.adder.load(annotation, s.interactionPoint, true);
                } else {
                    s.adder.hide();
                }
            },
            onSelectionStart: function (ranges, event){
                triggerParentAnnotationEvent("hideFocus", { }, options.parentAnnotationListenerSelector);
                s.adder.turnHighlight('Default');
            }
        });
        ...
        s.viewer = new viewer.Viewer({
            ...
            onClick: function(annotation, event){
                s.interactionPoint = util.mousePosition(event);
                s.adder.load(annotation, s.interactionPoint, false);
            },
    ...
    return {
        ...
        beforeAnnotationCreated: function (annotation) {
            ...
            if(window.rendition && window.rendition.currentLocation){
                var currentLocation = window.rendition.currentLocation();

                annotation['cfi'] = currentLocation.start.cfi
            }

            return s.editor.load(annotation, s.interactionPoint, true);
        },

        beforeAnnotationUpdated: function (annotation) {
            return s.editor.load(annotation, s.interactionPoint, false);
        }
    };
}
...
},{"../util":39,"./adder":29,"./editor":30,"./highlighter":32,"./textselector":36,"./viewer":37,"xpath-range":18}],34:[function(require,module,exports){
(function (global){
...
function TextSelector(element, options) {
    ...
    this.onSelectionStart = this.options.onSelectionStart;
    ...
    if (typeof this.element.ownerDocument !== 'undefined' &&
        ...
        $(this.document.body)
            .on("mouseup." + TEXTSELECTOR_NS, function (e) {
                if(e.target.className == 'highlightSpan')
                    return false;
...
TextSelector.prototype._turnHighlightSelected = function (event){
    var self = this;
...
TextSelector.prototype._checkForEndSelection = function (event) {
    ...
    var _nullSelection = function () {
        if (typeof self.onSelection === 'function') {
            self.onSelection([], event, TextSelector.options.selectedRanges);
        }
    };
    ...
    TextSelector.options.selectedRanges = selectedRanges;
...
var Viewer = exports.Viewer = Widget.extend({
    ...
    constructor: function (options) {
        ...
        if (this.options.autoViewHighlights) {
            ...
            $(this.options.autoViewHighlights).children().each(function(i, j) {
				if(typeof i == 'number'){
				$(j).on("click mouseover", '.annotator-hl', function (event) {
        ...
        if(window.platform == 'web' || window.article){
            if (typeof position !== 'undefined' && position !== null) {
                ...
                if (typeof position !== 'undefined' && position !== null) {
					...
					$(this.element).find('.annotator-widget').css('min-width', '200px')
					$(this.element).find('.annotator-widget').focus();
        ...
        this._startHideTimer(true)
            .done(function () {
                var annotations = $(event.target)
                    ...
                    .toArray();
                    if(annotations.length > 0 && annotations[0]["options"]){
                        
                                    self.options.onClick(annotations, event)
...
Widget.prototype.show = function () {
	...
	//this.turnHighlight(this.options.currentHighlight);
...
Widget.prototype.removeHighlight = function (classSuffix) {
    $(this.element).parent().removeClass('highlight' + this.options.currentHighlight);
...
Widget.prototype.turnHighlight = function (classSuffix) {

    $(this.element).parent().removeClass('highlight' + this.options.currentHighlight);
    
    if(classSuffix){
        this.options.currentHighlight = classSuffix;
    } else classSuffix = this.options.currentHighlight;
    
    $(this.element).parent().addClass('highlight' + classSuffix);
};
...
function reverseObject(obj){
    ...
    Object.keys(obj).forEach( function(i, j) {
        ret[obj[i]] = i
    })
