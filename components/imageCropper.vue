...
export default {
    ...
    mounted () {
		this.$root.$on('onImageUpload', ({image})=> {
			
			this.sourceImgUrl = image
		})
        		this.sourceImgUrl = this.myImage
				this.startCrop();
    },
	props: {
        myImage: {
	...
	methods: {
		...
		imgMove(e) {
			...
			scale.x = rX;
            scale.y = rY;
            // this.$emit('crop-success', this.createImgUrl, this.field, this.ki);
            this.$emit('crop-success', data2blob(this.createImgUrl, this.mime));
		},
		 // 按钮按下开始向右旋转
        startRotateRight(e) {
            let that = this,
...
.e-ripple {
  ...
  .e-ripple.z-active {
    ...
    .vue-image-crop-upload, .vue-image-crop-upload .vicp-wrap{ position: static}
